...
def build_question_analytics(results):
    ...
    for result in results:
        for q in result.get("question_results", []):
            rows.append({
                ...
                "Marks": float(q.get("marks_awarded", 0)),
                "Max Marks": float(q.get("max_marks", 0)),
            })
    ...
    grouped["Difficulty %"] = (
        grouped["Average_Marks"] /
        grouped["Average_Max"].replace(0, 1) *
        100
    ).round(1)

    return grouped[
        ["Question", "Average Marks", "Average Max Marks", "Difficulty %"]
    ]
...
def detect_review_flags(result, threshold):
    ...
    if float(result.get("confidence", 0)) < threshold:
        flags.append(
            f"AI confidence is below {threshold:.0%}."
        )
